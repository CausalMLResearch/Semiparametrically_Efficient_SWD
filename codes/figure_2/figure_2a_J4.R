...
results_long <- data.frame(
  r = rep(results$r, 4),
  pi_value = c(results$pi_1, results$pi_2, results$pi_3, results$pi_4),
  pi_type = factor(rep(c("pi1", "pi2", "pi3", "pi4"), each = nrow(results)), 
                   levels = c("pi1", "pi2", "pi3", "pi4"))
)
...
p <- ggplot(results_long, aes(x = r, y = pi_value, color = pi_type, group = pi_type)) +
  ...
  scale_color_rss_d(palette = "signif_qual", 
                    ...
                    labels = c(expression(pi[1]), expression(pi[2]), 
                               expression(pi[3]), expression(pi[4]))) +
...
ggsave("../../figures/figure_2/J_4.pdf", plot = p, 
