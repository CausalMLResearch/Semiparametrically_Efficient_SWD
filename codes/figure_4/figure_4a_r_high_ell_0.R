...
results_dir <- "../../results/figure_4/r_high_10_ell_0"
seeds <- 1:2000
true_effect <- 2
...
stats$pattern <- ifelse(stats$design %in% c("efficient_1", "efficient_2"), "stripe", "none")
...
p_mse <- ggplot(stats, aes(x = factor(x), y = mse)) +
  ...
  labs(x = "Design", 
       y = "MSE",
       title = "True Model: ℓ = 0") +
  theme_paper

print(p_mse)
ggsave("../../figures/figure_4/vary_ell_0.pdf", 
