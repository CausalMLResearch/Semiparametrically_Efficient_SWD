...
minimize_variance_bound_nloptr <- function(J, ell, r, sigma_epsilon, sigma_residual, sigma_phi) {
  
  objective_fn <- function(pi) {
    term1 <- 0
    for (j in (ell + 1):J) term1 <- term1 + (sigma_epsilon ** 2 + sigma_residual ** 2 / 2 + sigma_phi ** 2 / 2) / pi[j - ell] + (sigma_epsilon ** 2 + sigma_residual ** 2 / 2 + sigma_phi ** 2 / 2) / (1 - pi[j])
    ...
    if ((ell + 1) <= (J - 1)) {
      for (j in (ell + 1):(J - 1)) {
        for (jp in (j + 1):J) {
          if (j >= jp - ell) {
            term2 <- term2 + 2 * (sigma_epsilon ** 2 * r ** abs(jp - j) + sigma_phi ** 2 / 2) * (1 / pi[jp - ell] + 1 / (1 - pi[j]))
          }
        }
        if ((j + ell + 1) <= J) {
          for (jp in (j + ell + 1):J) {
            term2 <- term2 + 2 * (sigma_epsilon ** 2 * r ** abs(jp - j) + sigma_phi ** 2 / 2) * (1 / pi[jp - ell] + 1 / (1 - pi[j]) - (pi[jp - ell] - pi[j]) / ((1 - pi[j]) * pi[jp - ell]))
          }
        }
      }
    }
...
simulate_synthetic_panel <- function(df,
  ...
  df_historical <- df_window %>%
    ...
    mutate(id_individual = as.factor(id_individual))
  ...
  if (design_type == "efficient") {
    fit <- glmmTMB(out_cont_1 ~ time + ar1(time + 0 | id_unit) + (1 | id_individual), data = df_historical)
    ...
    sigma_phi <- unname(attr(VarCorr(fit)$cond$id_individual, "stddev")[1])
  ...
  treatment_lookup <- data.frame(
    id_unit = rep(1:I, each = J_exp),
    time = rep(1:J_exp, times = I),
    trt = as.vector(t(X))
  )
  
  treatment_lookup$time <- as.factor(treatment_lookup$time)
  ...
  df_synthetic$id_individual <- as.factor(df_synthetic$id_individual)
