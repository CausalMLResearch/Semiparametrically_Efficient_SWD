...
for (J in j_values) {
  results_dir <- sprintf("../../results/supp_figure_1/r_high_%d", J)
...
p_mse_line <- ggplot(combined_data, aes(x = J, y = mse, color = design, fill = design)) +
  ...
  scale_y_continuous(limits = c(0.00148, 0.0042), 
                     breaks = seq(0.0015, 0.004, length.out = 6),
                     expand = c(0, 0)) +
  labs(x = "J", y = "MSE") +
...
ggsave("../../figures/supp_figure_1/vary_J_1000.pdf", p_mse_line, width = 6, height = 4, dpi = 300, device = cairo_pdf)