...
results_dir <- "../../results/supp_figure_1/r_high_10_ell_1"
seeds <- 1:2000
true_effect <- 2
...
stats$pattern <- ifelse(stats$design %in% c("efficient_0", "efficient_2"), "stripe", "none")
...
stats$fill_color <- c(
  ...
  "white",           # efficient_0 (with stripe)
  design_colors[4],  # efficient_1
  "white"            # efficient_2 (with stripe)
)
...
p_mse <- ggplot(stats, aes(x = factor(x), y = mse)) +
  ...
  labs(x = "Design", 
       y = "MSE",
       title = "True Model: ℓ = 1") +
  theme_paper

print(p_mse)
ggsave("../../figures/supp_figure_1/vary_ell_1_1000.pdf", 
