...
results_dir <- "../../results/supp_figure_1/multiple_estimands"
seeds <- 1:2000
...
create_mse_plot <- function(plot_data, y_label, plot_title, y_max = NULL) {
  ...
  p <- ggplot(plot_data, aes(x = factor(x), y = mse)) +
    ...
    scale_y_continuous(limits = c(0, y_max), expand = c(0, 0), n.break = 5) +
...
ggsave("../../figures/supp_figure_1/multiple_gate_1000.pdf", 
       p_gate, width = 6, height = 4, 
       device = cairo_pdf)
ggsave("../../figures/supp_figure_1/multiple_cte_1000.pdf", 
       p_cte, width = 6, height = 4, 
       device = cairo_pdf)
ggsave("../../figures/supp_figure_1/multiple_both_1000.pdf", 
