...
results_dir <- "../../results/supp_figure_1/r_high_10"
seeds <- 1:2000
true_effect <- 2
all_results <- list()
...
stats$x <- 1:6
stats$pattern <- ifelse(stats$design %in% c("efficient_7", "efficient_5"), 
                        "stripe", "none")
...
stats$fill_color <- c(
  design_colors_base[1],
  design_colors_base[2],
  design_colors_base[3],
...
p_single <- ggplot(stats, aes(x = factor(x), y = mse)) +
  ...
  scale_y_continuous(limits = c(0, 0.0042), expand = c(0, 0)) +
  labs(x = "Design", 
       y = "MSE",
       title = "True Model: r = 0.9") +
  theme_paper

ggsave("../../figures/supp_figure_1/vary_r_0.9_1000.pdf", 
