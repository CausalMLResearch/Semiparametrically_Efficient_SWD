  ...
  objective_fn <- function(pi) {
    ...
    if ((ell + 1) <= (J - 1)) {
      for (j in (ell + 1):(J - 1)) {
        for (jp in (j + 1):J) {
          if (j >= jp - ell) {
            term2 <- term2 + 2 * (sigma_epsilon ** 2 * r ** abs(jp - j) + sigma_residual ** 2) / (sigma_epsilon ** 2 + sigma_residual ** 2) * (1 / pi[jp - ell] + 1 / (1 - pi[j]))
          }
        }
        if ((j + ell + 1) <= J) {
          for (jp in (j + ell + 1):J) {
            term2 <- term2 + 2 * (sigma_epsilon ** 2 * r ** abs(jp - j) + sigma_residual ** 2) / (sigma_epsilon ** 2 + sigma_residual ** 2) * (1 / pi[jp - ell] + 1 / (1 - pi[j]) - (pi[jp - ell] - pi[j]) / ((1 - pi[j]) * pi[jp - ell]))
          }
        }
      }
    }
...
simulate_synthetic_panel <- function(df,
  ...
  efficient_matrix <- NULL
  correlation_params <- NULL  # Initialize correlation parameters
  ...
  attr(df_synthetic, "correlation_params") <- correlation_params  # Add correlation parameters
